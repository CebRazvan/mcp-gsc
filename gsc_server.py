...
def main():
    ...
    if transport == "stdio":
        ...
        # Note: a large OAuth-discovery / DCR / JWT stub lived here
        # in previous revisions as an attempt to make Claude.ai's web
        # MCP connector work. Claude does deep internal validation of
        # the OAuth provider that a stub cannot satisfy — see git
        # history and DEPLOY.md for the journey. The supported remote
        # path is now: MCP_BEARER_TOKEN + mcp-remote (stdio→HTTP
        # bridge) in Claude Desktop, or any client that accepts a
        ...
        async def _serve():
            ...
            # Stack inside-out: MCP → bearer (if set) → accept_shim → debug.
            # Accept shim rewrites Accept: */* to the spec-compliant
            # value so the SDK's strict prefix-matcher doesn't 406
            # clients (e.g. mcp-remote, curl -X POST without headers).
            if bearer_token:
